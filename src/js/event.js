...
var events = {
    ...
    handleStartBtnClick: function() {
        ...
        playingUnit.style.background = 'url(./src/images/0.jpg)';
        ...
        gameWrapper.onclick = function(e) {
            ...
            events.handlePlayMusic(flyMusic, 'fly.wav');
        };
        // 按下空格键,小鸟同样上移
    ...
    handleBlockMove: function() {
        if (blocksArr.length) {
            ...
            if (blocksArr[0].upBlockWrapper.offsetLeft == -12) {
                 events.handlePlayMusic(infoMusic, 'info.wav');
                score++;
                if (score < 10) {
                    unit = score;

                    playingUnit.style.display = 'block';
                    playingUnit.style.background = 'url(./src/images/' + unit + '.jpg)';
                    ...
                    playingDecade.style.background = 'url(./src/images/' + decade + '.jpg)';
                    playingUnit.style.background = 'url(./src/images/' + unit + '.jpg)';
                    ...
                    playingCentile.style.background = 'url(./src/images/' + centile + '.jpg)';
                    playingDecade.style.background = 'url(./src/images/' + decade + '.jpg)';
                    playingUnit.style.background = 'url(./src/images/' + unit + '.jpg)';
                }
            }
    ...
    handleGameOver: function() {
        ...
        events.handlePlayMusic(crashMusic, 'crash.wav');
     ...
     * @param src 
     */
    handlePlayMusic: function(element, url) {

        if(!element.src){
            element.src = musicUrl +  url;
        }
        element.play();
    }
};